...
use microservice::{
    config::Config,
    middleware::{HttpCacheMiddleware, JwtAuthMiddleware, RequestValidationMiddleware},
    opentelemetry::{OpenTelemetryConfig, OpenTelemetryManager},
    security::{SecurityConfig, SecurityManager},
    service_mesh::{ServiceInstance, ServiceMeshConfig, ServiceMeshManager},
};
...
fn benchmark_jwt_auth_middleware(c: &mut Criterion) {
    c.bench_function("jwt_auth_middleware_creation", |b| {
        b.iter(|| {
            let _middleware =
                JwtAuthMiddleware::new(microservice::middleware::JwtConfig::default());
            black_box(())
        })
    });
}
...
fn benchmark_request_validation_middleware(c: &mut Criterion) {
    c.bench_function("request_validation_middleware_creation", |b| {
        b.iter(|| {
            let _middleware = RequestValidationMiddleware::new(
                microservice::middleware::ValidationConfig::default(),
            );
            black_box(())
        })
    });
...
fn benchmark_cache_middleware(c: &mut Criterion) {
    c.bench_function("cache_middleware_creation", |b| {
        b.iter(|| {
            let _middleware =
                HttpCacheMiddleware::new(microservice::middleware::CacheConfig::default());
            black_box(())
        })
    });
}
