...
/// 演示熔断器和限流器
async fn demo_circuit_breaker_and_rate_limiter() -> std::result::Result<(), Box<dyn std::error::Error>> {
    println!("\n⚡ 演示熔断器和限流器");
    println!("-------------------");
    ...
    // 模拟失败请求
    for i in 1..=4 {
        let result = circuit_breaker.call(|| -> std::result::Result<String, EnhancedServiceError> {
            println!("   执行失败操作 {}", i);
            Err(EnhancedServiceError::ServiceUnavailable("模拟服务不可用".to_string()))
        }).await;
...
async fn demo_service_registry() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, health) in all_health {
        println!("   {}: {:?} (运行{}秒)", name, health.status, health.uptime_seconds);
    }
