...
impl SimpleMicroserviceDemoManager {
    ...
    pub async fn demo_service_registry(&self) -> Result<()> {
        ...
        // 注册多个服务
        let user_service = SimpleUserService::new();
        registry.register("user-service".to_string(), Box::new(user_service)).await?;
    ...
    pub async fn demo_performance_monitoring(&self) -> Result<()> {
        ...
        // 记录一些性能指标
        monitor.record_metric("request_count".to_string(), 100.0).await?;
        monitor.record_metric("response_time_ms".to_string(), 50.0).await?;
        monitor.record_metric("error_rate".to_string(), 0.01).await?;
        monitor.record_metric("cpu_usage".to_string(), 75.5).await?;
        monitor.record_metric("memory_usage_mb".to_string(), 512.0).await?;
