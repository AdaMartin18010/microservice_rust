...
impl FrameworkDemoManager {
    ...
    pub fn register_all_frameworks(&mut self) -> Result<()> {
        ...
        #[cfg(feature = "with-spring-rs")]
        {
            ...
            self.services.insert("Spring-rs".to_string(), spring_rs_service);
    ...
    pub async fn demo_concurrent_processing(&self) -> Result<()> {
        ...
        let handles: Vec<_> = self.services.iter()
            .map(|(name, service)| {
                let name = name.clone();
                tokio::spawn(async move {
                    ...
                    match metrics_result {
                        Ok(metrics) => println!("  - 指标获取: 成功 ({}ms) - 请求数: {}", metrics_time, metrics.requests_total),
