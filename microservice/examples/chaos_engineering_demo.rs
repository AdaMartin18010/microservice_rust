...
impl ChaosEngineeringDemoManager {
    ...
    pub async fn demo_fault_injection(&self) -> Result<()> {
        ...
        // 网络延迟故障
        println!("注入网络延迟故障...");
        fault_injector.inject_network_latency("user_service", 500).await?;
        
        // 服务不可用故障
        println!("\n注入服务不可用故障...");
        fault_injector.inject_service_unavailable("order_service", 10).await?;
        
        // 资源耗尽故障
        println!("\n注入资源耗尽故障...");
        fault_injector.inject_resource_exhaustion("payment_service", "memory").await?;
        
        // 依赖故障
        println!("\n注入依赖故障...");
        fault_injector.inject_dependency_failure("notification_service", "email_service").await?;
        
        // 数据损坏故障
        println!("\n注入数据损坏故障...");
        fault_injector.inject_data_corruption("database_service", "user_data").await?;
    ...
    pub async fn demo_resilience_testing(&self) -> Result<()> {
        ...
        // 运行弹性测试
        let test_results = resilience_tester.run_resilience_test("系统弹性测试", 60).await?;
        ...
        println!("  - 平均响应时间: {:.2}ms", test_results.average_response_time);
    ...
    pub async fn demo_chaos_configuration(&self) -> Result<()> {
        ...
        println!("当前混沌工程配置:");
        println!("  - 混沌猴子: {}", if config.enable_chaos_monkey { "启用" } else { "禁用" });
        ...
        println!("  - 自动恢复: {}", if config.enable_auto_recovery { "启用" } else { "禁用" });
        println!("  - 监控: {}", if config.monitoring_enabled { "启用" } else { "禁用" });
