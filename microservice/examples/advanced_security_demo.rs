...
impl AdvancedSecurityDemoManager {
    ...
    pub async fn demo_zero_trust(&self) -> Result<()> {
        ...
        // 演示权限检查
        println!("\n权限检查测试:");
        let has_permission = self.service.check_permission("admin", "/api/admin/*", "read").await?;
        println!("管理员访问 /api/admin/*: {}", if has_permission { "✅ 允许" } else { "❌ 拒绝" });
        
        let has_permission = self.service.check_permission("admin", "/api/user/*", "write").await?;
        println!("管理员访问 /api/user/*: {}", if has_permission { "✅ 允许" } else { "❌ 拒绝" });
        
        // 演示失败认证
        println!("\n失败认证测试:");
        let token = self.service.authenticate_user("admin", "wrong_password").await?;
        match token {
    ...
    pub async fn demo_oauth2_flow(&self) -> Result<()> {
        ...
        // 步骤1: 生成授权码
        println!("步骤1: 生成授权码");
        let auth_code = self.service.generate_oauth2_auth_code(
            client_id,
            user_id,
            redirect_uri,
            scope,
        ...
        // 步骤2: 交换访问令牌
        println!("\n步骤2: 交换访问令牌");
        let access_token = self.service.exchange_oauth2_token(
            &auth_code,
            client_id,
            client_secret,
        ).await?;
        ...
        // 演示无效客户端
        println!("\n无效客户端测试:");
        let invalid_token = self.service.exchange_oauth2_token(
            &auth_code,
            "invalid_client",
            "invalid_secret",
        ).await?;
    ...
    pub async fn demo_rbac_management(&self) -> Result<()> {
        ...
        self.service.assign_user_role(user_id, Role::Moderator).await?;
        println!("✅ 分配 Moderator 角色");
        
        // 检查权限 - 使用公共方法
        println!("\n权限检查:");
        let has_read = self.service.check_user_permission(user_id, &Permission::Read).await?;
        println!("读取权限: {}", if has_read { "✅ 有权限" } else { "❌ 无权限" });
        
        let has_write = self.service.check_user_permission(user_id, &Permission::Write).await?;
        println!("写入权限: {}", if has_write { "✅ 有权限" } else { "❌ 无权限" });
        
        let has_admin = self.service.check_user_permission(user_id, &Permission::Admin).await?;
        println!("管理员权限: {}", if has_admin { "✅ 有权限" } else { "❌ 无权限" });
        
        let has_user_mgmt = self.service.check_user_permission(user_id, &Permission::UserManagement).await?;
        println!("用户管理权限: {}", if has_user_mgmt { "✅ 有权限" } else { "❌ 无权限" });
        
        // 检查角色 - 使用公共方法
        println!("\n角色检查:");
        let has_user_role = self.service.check_user_role(user_id, &Role::User).await?;
        println!("User 角色: {}", if has_user_role { "✅ 有角色" } else { "❌ 无角色" });
        
        let has_admin_role = self.service.check_user_role(user_id, &Role::Admin).await?;
        println!("Admin 角色: {}", if has_admin_role { "✅ 有角色" } else { "❌ 无角色" });
        ...
        let has_user_role = self.service.check_user_role(user_id, &Role::User).await?;
        println!("User 角色 (移除后): {}", if has_user_role { "✅ 有角色" } else { "❌ 无角色" });
    ...
    pub async fn demo_mtls(&self) -> Result<()> {
        ...
        println!("客户端证书验证: {}", if is_valid { "✅ 有效" } else { "❌ 无效" });
    ...
    pub async fn demo_security_monitoring(&self) -> Result<()> {
        ...
        let success_events = events.iter().filter(|e| e.result == EventResult::Success).count();
        let failure_events = events.iter().filter(|e| e.result == EventResult::Failure).count();
        let blocked_events = events.iter().filter(|e| e.result == EventResult::Blocked).count();
    ...
    pub async fn demo_security_policies(&self) -> Result<()> {
        ...
        for (policy_id, policy) in policies.iter() {
            ...
            println!("  - 状态: {}", if policy.is_active { "激活" } else { "停用" });
    ...
    pub async fn demo_security_configuration(&self) -> Result<()> {
        ...
        println!("当前安全配置:");
        println!("  - 零信任架构: {}", if config.enable_zero_trust { "启用" } else { "禁用" });
        println!("  - mTLS: {}", if config.enable_mtls { "启用" } else { "禁用" });
        println!("  - OAuth2: {}", if config.enable_oauth2 { "启用" } else { "禁用" });
        println!("  - RBAC: {}", if config.enable_rbac { "启用" } else { "禁用" });
