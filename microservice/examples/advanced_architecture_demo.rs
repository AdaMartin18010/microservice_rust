...
async fn demo_cqrs_event_sourcing() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, event) in all_events.iter().enumerate() {
        println!("     {}. {} - 版本 {}", i + 1, event.event_type(), event.version());
    }
...
async fn demo_chaos_engineering() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    chaos.register_fault_injector(Box::new(latency_injector)).await;
...
async fn demo_auto_scaling() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for (scenario_name, cpu_usage, current_instances) in scenarios {
        println!("\n✅ 场景: {} (CPU: {}%, 当前实例: {})", scenario_name, cpu_usage, current_instances);
        
        // 更新指标
        auto_scaling.update_metric("cpu_usage".to_string(), ArchMetricValue {
            value: cpu_usage,
            timestamp: chrono::Utc::now(),
            unit: "percent".to_string(),
        }).await;
        
        auto_scaling.update_metric("instance_count".to_string(), ArchMetricValue {
