...
use microservice::{
    error::Result,
    middleware::{
        DistributedTracingConfig, DistributedTracingMiddleware, TraceContextBuilder,
        TracingLogLevel,
    },
    opentelemetry::{OpenTelemetryConfig, OpenTelemetryManager},
};
...
impl DistributedTracingDemo {
    ...
    async fn demonstrate_http_tracing(&mut self) -> Result<()> {
        ...
        headers.insert(
            ...
            "distributed-tracing-demo/1.0.0".to_string(),
        );
        ...
        // 完成HTTP请求追踪
        self.tracing_middleware.trace_http_response(
            span,
            200,
        ...
        let (_, error_span) =
            self.tracing_middleware
                .trace_http_request("POST", "/api/users", &headers);
    ...
    async fn demonstrate_database_tracing(&mut self) -> Result<()> {
        ...
        for (query, rows_affected) in queries {
            let span = self
                .tracing_middleware
                .start_span("database_query", current_context.clone());
            ...
            if success {
                self.tracing_middleware.trace_database_query(
                    span,
                    query,
                    duration,
                    rows_affected,
    ...
    async fn demonstrate_external_service_tracing(&mut self) -> Result<()> {
        ...
        for (service_name, endpoint, method) in services {
            ...
            let span = self
                .tracing_middleware
                .start_span("external_service_call", current_context);
    ...
    async fn demonstrate_context_propagation(&mut self) -> Result<()> {
        ...
        if let Some(context) = extracted_context {
            ...
            // 创建子跨度
            let child_context = TraceContextBuilder::new().with_parent(&context).build();
            ...
            let span = self
                .tracing_middleware
                .start_span("child_operation", Some(child_context));
            if let Some(span) = span {
    ...
    async fn demonstrate_custom_event_tracing(&mut self) -> Result<()> {
        ...
        let events = vec![
            ...
            (
                "payment_processed",
                vec![
                    ("payment_id".to_string(), "PAY-001".to_string()),
        ...
        for (event_name, attributes) in events {
            ...
            let span = self
                .tracing_middleware
                .start_span("custom_event", current_context);
            ...
            if success {
                self.tracing_middleware
                    .trace_custom_event(span, event_name, Some(attrs_map), None);
            } else {
                self.tracing_middleware.trace_custom_event(
                    span,
                    event_name,
                    Some(attrs_map),
                    Some("Event processing failed"),
                );
            }
        }
