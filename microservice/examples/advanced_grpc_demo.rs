...
use microservice::grpc::{
    AdvancedGrpcClient, AdvancedUserService, BatchUserRequest, BatchUserResponse, OperationResult,
    UserOperation,
};
...
async fn demo_advanced_user_service() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user2 = service
        .create_user("李四".to_string(), "lisi@example.com".to_string())
        .await?;
    ...
    println!(
        "✅ 获取用户: {} (版本: {})",
        retrieved_user.name, retrieved_user.version
    );
    ...
    println!(
        "✅ 更新用户: {} (版本: {})",
        updated_user.name, updated_user.version
    );
...
async fn demo_version_control() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✅ 第一次更新成功: {} (版本: {})",
        updated_user1.name, updated_user1.version
    );
    ...
    println!(
        "✅ 第三次更新成功: {} (版本: {})",
        updated_user3.name, updated_user3.version
    );
...
async fn demo_grpc_client() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match AdvancedGrpcClient::new("http://localhost:50051").await {
        Ok(mut client) => {
            ...
            // 创建用户
            let user = client
                .create_user("客户端用户".to_string(), "client@example.com".to_string())
                .await?;
            ...
            // 批量操作
            let batch_request = BatchUserRequest {
                operations: vec![UserOperation::Create {
...
async fn demo_error_handling() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let user = service
        .create_user(
            "冲突测试用户".to_string(),
            "conflict@example.com".to_string(),
        )
        .await?;

    println!("测试版本冲突...");
    match service
        .update_user(&user.id, Some("冲突更新".to_string()), None, 999)
        .await
    {
