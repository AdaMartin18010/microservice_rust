...
impl AIMLDemoManager {
    ...
    pub async fn initialize(&self) -> Result<()> {
        ...
        self.service.register_text_model("bert-base".to_string(), text_model).await?;
        ...
        for (i, text) in test_texts.iter().enumerate() {
            ...
            if let AIMLResult::TextAnalysis { sentiment, .. } = response.result {
                if let Some(sentiment) = sentiment {
                    println!("  💭 情感分析: {} (置信度: {:.2})", sentiment.sentiment, sentiment.score);
                }
            }
    ...
    pub async fn demo_image_analysis(&self) -> Result<()> {
        ...
        for (task_name, task) in image_tasks {
            ...
            match response.result {
                AIMLResult::ImageAnalysis { classification, objects, faces, extracted_text, scene, .. } => {
            ...
            for item_type in &item_types {
                ...
                if let AIMLResult::Recommendation { items, reasoning } = response.result {
                    ...
                    for (i, item) in items.iter().enumerate() {
                        println!("    {}. {} (分数: {:.2}) - {}", i + 1, item.item_id, item.score, item.reason);
                    }
    ...
    pub async fn demo_anomaly_detection(&self) -> Result<()> {
        ...
        let test_datasets = vec![
            ...
            ("网络延迟", vec![10.0, 12.0, 15.0, 11.0, 13.0, 100.0, 14.0, 12.5]), // 包含网络异常
        ];
        ...
        for (dataset_name, data) in test_datasets {
            ...
            if let AIMLResult::AnomalyDetection { is_anomaly, anomaly_score, explanation } = response.result {
                let status = if is_anomaly { "🚨 异常" } else { "✅ 正常" };
    ...
    pub async fn demo_batch_processing(&self) -> Result<()> {
        ...
        println!("  - 吞吐量: {:.2} req/s", (success_count as f64 * 1000.0) / total_time_elapsed as f64);
    ...
    pub async fn demo_configuration_management(&self) -> Result<()> {
        ...
        println!("  - 推理超时时间: {}ms", self.service.config.inference_timeout_ms);
        println!("  - 批处理大小: {}", self.service.config.batch_size);
        println!("  - GPU 加速: {}", self.service.config.enable_gpu);
        println!("  - 模型更新间隔: {}s", self.service.config.model_update_interval);
