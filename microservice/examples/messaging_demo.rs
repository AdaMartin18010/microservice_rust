...
use microservice::messaging::{
    Acks, Compression, KafkaConsumerConfig, KafkaManager, KafkaProducerConfig, Message,
    MessageHandler, NatsConfig, NatsManager,
};
...
async fn demo_kafka() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    let messages = vec![
        (
            Some(b"key1".to_vec()),
            b"Hello Kafka 1".to_vec(),
            None::<HashMap<String, Vec<u8>>>,
        ),
        (
            Some(b"key2".to_vec()),
            b"Hello Kafka 2".to_vec(),
            None::<HashMap<String, Vec<u8>>>,
        ),
        (
            Some(b"key3".to_vec()),
            b"Hello Kafka 3".to_vec(),
            None::<HashMap<String, Vec<u8>>>,
        ),
    ];
...
async fn demo_nats() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for (i, payload) in payloads.iter().enumerate() {
        ...
        nats_manager
            .publish_message(&subject, payload, None)
            .await?;
        tokio::time::sleep(Duration::from_millis(100)).await;
    }
    ...
    let _subscription = nats_manager
        .subscribe_topic("demo.subject.*", handler)
        .await?;

    // 获取统计信息
    if let Some(stats) = nats_manager.get_stats() {
        info!(
            "NATS统计信息: 入站消息={}, 出站消息={}, 订阅数={}",
...
async fn performance_test() -> std::result::Result<(), Box<dyn std::error::Error>> {
    ...
    for i in 0..message_count {
        ...
        if let Err(e) = kafka_manager
            .publish_message(&topic, Some(b"key"), &payload, None)
            .await
        {
            error!("发布消息失败: {}", e);
        }
    ...
    info!(
        "  带宽: {:.2} MB/s",
        (message_count * payload_size) as f64 / duration.as_secs_f64() / 1024.0 / 1024.0
    );
