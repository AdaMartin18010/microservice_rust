...
impl MultiCloudDemoManager {
    ...
    pub async fn demo_aws_integration(&self) -> Result<()> {
        ...
        let ec2_instance = self
            .manager
            .create_compute_resource(CloudProvider::Aws, "demo-ec2-instance", "t3.micro")
            .await?;
        ...
        let s3_bucket = self
            .manager
            .create_storage_resource(CloudProvider::Aws, "demo-s3-bucket", "us-east-1")
            .await?;
        ...
        // 创建 RDS 数据库实例
        println!("\n创建 AWS RDS 数据库实例...");
        let rds_instance = self
            .manager
            .create_database_resource(CloudProvider::Aws, "demo-rds-instance", "MySQL")
            .await?;
    ...
    pub async fn demo_azure_integration(&self) -> Result<()> {
        ...
        let vm = self
            .manager
            .create_compute_resource(CloudProvider::Azure, "demo-azure-vm", "Standard_B1s")
            .await?;
        ...
        let storage_account = self
            .manager
            .create_storage_resource(CloudProvider::Azure, "demo-storage-account", "eastus")
            .await?;
        ...
        let sql_db = self
            .manager
            .create_database_resource(CloudProvider::Azure, "demo-sql-database", "SQL Server")
            .await?;
    ...
    pub async fn demo_gcp_integration(&self) -> Result<()> {
        ...
        let compute_instance = self
            .manager
            .create_compute_resource(CloudProvider::Gcp, "demo-gcp-compute", "e2-micro")
            .await?;
        ...
        // 创建云存储桶
        println!("\n创建 GCP 云存储桶...");
        let cloud_storage = self
            .manager
            .create_storage_resource(CloudProvider::Gcp, "demo-gcp-storage", "us-central1")
            .await?;
        ...
        let cloud_sql = self
            .manager
            .create_database_resource(CloudProvider::Gcp, "demo-gcp-sql", "MySQL")
            .await?;
    ...
    pub async fn demo_alibaba_integration(&self) -> Result<()> {
        ...
        let rds_instance = self
            .manager
            .create_database_resource(CloudProvider::AlibabaCloud, "demo-aliyun-rds", "MySQL")
            .await?;
    ...
    pub async fn demo_multi_cloud_management(&self) -> Result<()> {
        ...
        for provider in [
            ...
            let provider_resources = self
                .manager
                .get_resources_by_provider(provider.clone())
                .await;
        ...
        for resource_type in [
            CloudResourceType::Compute,
            CloudResourceType::Storage,
            CloudResourceType::Database,
        ] {
            let type_resources = self
                .manager
                .get_resources_by_type(resource_type.clone())
                .await;
    ...
    pub async fn demo_multi_cloud_costs(&self) -> Result<()> {
        ...
        println!(
            "  - 总成本: ${:.2} {}",
            cost_summary.total_cost, cost_summary.currency
        );
        println!(
            "  - 统计期间: {} 到 {}",
        ...
        if cost_summary.provider_costs.len() > 1 {
            let min_provider = cost_summary
                .provider_costs
                .iter()
                .min_by(|a, b| a.1.partial_cmp(b.1).unwrap())
                .unwrap();
            let max_provider = cost_summary
                .provider_costs
                .iter()
            ...
            println!(
                "  - 最低成本提供商: {:?} (${:.2})",
                min_provider.0, min_provider.1
            );
            println!(
                "  - 最高成本提供商: {:?} (${:.2})",
                max_provider.0, max_provider.1
            );
    ...
    pub async fn demo_multi_cloud_migration(&self) -> Result<()> {
        ...
        println!(
            "选择迁移资源: {} (当前在 {:?})",
            source_resource.name, source_resource.provider
        );
        ...
        let target_provider = target_providers
            .iter()
        ...
        // 执行迁移
        println!("\n开始迁移...");
        let migrated_resource = self
            .manager
            .migrate_resource(&source_resource.id, target_provider.clone())
            .await?;
        ...
        println!(
            "  - 目标资源: {} ({:?})",
            migrated_resource.name, migrated_resource.provider
        );
