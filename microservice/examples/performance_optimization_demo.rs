...
impl PerformanceOptimizationDemoManager {
    pub fn new() -> Self {
        ...
        // 设置并发优化器
        let concurrency_config = PerformanceOptimizationFactory::create_default_concurrency_config();
    ...
    async fn compare_with_targets(&self, result: &BenchmarkResult) -> Result<()> {
        ...
        if let Some(target_throughput) = result.config.target_throughput {
            ...
            println!("吞吐量目标: {:.0} req/s, 实际: {:.2} req/s, 达成率: {:.1}%", 
        ...
        if let Some(target_response_time) = result.config.target_response_time {
            ...
            println!("响应时间目标: {:.0} ms, 实际: {:.2} ms, 比率: {:.2}x", 
    ...
    pub async fn demo_memory_optimization(&self) -> Result<()> {
        ...
        println!("  - 内存碎片: {:.2}%", before_stats.memory_fragmentation_percent);
        ...
        println!("  - 内存碎片: {:.2}%", after_stats.memory_fragmentation_percent);
        
        // 分析优化效果
        self.analyze_memory_optimization(&optimization_result).await?;
    ...
    async fn analyze_memory_optimization(&self, result: &MemoryOptimizationResult) -> Result<()> {
        ...
        let memory_saved_percent = (result.memory_saved_mb / result.start_stats.used_memory_mb) * 100.0;
        println!("内存节省: {:.2} MB ({:.1}%)", result.memory_saved_mb, memory_saved_percent);
    ...
    pub async fn demo_concurrency_optimization(&self) -> Result<()> {
        ...
        println!("  - 线程池利用率: {:.1}%", before_stats.thread_pool_utilization);
        ...
        println!("  - 线程池利用率: {:.1}%", after_stats.thread_pool_utilization);
        
        // 分析优化效果
        self.analyze_concurrency_optimization(&optimization_result).await?;
        
        Ok(())
    }
    
    /// 分析并发优化效果
    async fn analyze_concurrency_optimization(&self, result: &ConcurrencyOptimizationResult) -> Result<()> {
        println!("\n📈 并发优化效果分析:");
        println!("================================");
        ...
        let optimization_efficiency = result.throughput_improvement_percent / result.optimization_time_ms * 1000.0;
        ...
        let overall_improvement = (result.throughput_improvement_percent + 
                                 ...
                                 result.error_rate_reduction_percent) / 3.0;
    ...
    pub async fn demo_comprehensive_optimization(&self) -> Result<()> {
        ...
        // 运行全面优化
        let comprehensive_result = self.manager.run_comprehensive_optimization(test_function).await?;
        
        // 分析全面优化结果
        self.analyze_comprehensive_optimization(&comprehensive_result).await?;
        
        Ok(())
    }
    
    /// 分析全面优化结果
    async fn analyze_comprehensive_optimization(&self, result: &ComprehensiveOptimizationResult) -> Result<()> {
        println!("\n📈 全面优化结果分析:");
        println!("================================");
        ...
        for optimization_result in &result.results {
            match optimization_result {
                OptimizationResult::Benchmark(benchmark) => {
                    ...
                    println!("  - 平均响应时间: {:.2} ms", benchmark.average_response_time);
                ...
                OptimizationResult::Concurrency(concurrency) => {
                    println!("并发优化结果:");
                    println!("  - 吞吐量提升: {:.1}%", concurrency.throughput_improvement_percent);
                    println!("  - 延迟减少: {:.1}%", concurrency.latency_reduction_percent);
                }
            }
        }
    ...
    pub async fn demo_performance_recommendations(&self) -> Result<()> {
        ...
        for (i, recommendation) in recommendations.iter().enumerate() {
            println!("{}. {} ({})", i + 1, recommendation.title, recommendation.category);
        ...
        for (i, recommendation) in sorted_recommendations.iter().enumerate() {
            println!("{}. {} ({:?})", i + 1, recommendation.title, recommendation.priority);
        }
