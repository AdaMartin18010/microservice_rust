...
pub trait AggregateRoot {
    ...
    ) -> Result<Vec<Self::Event>, ArchitectureError>;
...
pub trait EventStore {
    ...
    async fn get_events(
        ...
        aggregate_id: &str,
        from_version: u64,
    ) -> Result<Vec<DomainEvent>, ArchitectureError>;
...
impl EventStore for InMemoryEventStore {
    async fn save_events(
        ...
        // 检查版本一致性
        let current_version = aggregate_events
            .get(aggregate_id)
            .map(|events| events.len() as u64)
            .unwrap_or(0);

        if current_version != expected_version {
            return Err(ArchitectureError::DomainError(format!(
                "版本冲突: 期望 {}, 实际 {}",
        ...
        // 保存事件
        let aggregate_events_list = aggregate_events
        ...
        match command {
            ...
            UserCommand::UpdateUser { name, email } => {
                if self.status == UserStatus::Active {
                    ...
                    Err(ArchitectureError::DomainError(
                        "只能更新活跃用户".to_string(),
                    ))
                }
            }
...
impl CommandHandler<UserCommand> for UserCommandHandler {
    async fn handle(&self, command: UserCommand) -> Result<Vec<DomainEvent>, ArchitectureError> {
        let aggregate_id = match &command {
            ...
            _ => {
                return Err(ArchitectureError::CommandError(
                    "需要指定聚合ID".to_string(),
                ));
            }
        };
        ...
        // 转换为领域事件
        let domain_events: Vec<DomainEvent> = domain_events
            .into_iter()
        ...
        self.event_store
            .save_events(
                &aggregate_id,
                domain_events.clone(),
                aggregate.get_version() - domain_events.len() as u64,
            )
            .await?;
...
impl Default for UserQueryHandler {
    fn default() -> Self {
        Self::new()
    }
}
...
impl UserQueryHandler {
    ...
    pub async fn update_read_model(&self, event: &DomainEvent) -> Result<(), ArchitectureError> {
        if let Ok(user_event) = serde_json::from_value::<UserEvent>(event.event_data.clone()) {
            ...
            let mut user = read_model
                .get(&event.aggregate_id)
                .cloned()
                .unwrap_or_else(|| UserAggregate {
...
impl QueryHandler<GetUserQuery, Option<UserAggregate>> for UserQueryHandler {
    async fn handle(
        &self,
        query: GetUserQuery,
    ) -> Result<Option<UserAggregate>, ArchitectureError> {
...
impl SagaCoordinator {
    ...
    pub async fn execute_saga(&self, saga_id: &str) -> Result<(), ArchitectureError> {
        ...
        if let Some(saga) = sagas.get(saga_id) {
            ...
            Err(ArchitectureError::SagaError(format!(
                "Saga {} 未找到",
                saga_id
            )))
        }
    ...
    async fn execute(&self) -> Result<(), ArchitectureError> {
        for step in &self.steps {
            ...
            match step.action.as_str() {
                ...
                _ => {
                    return Err(ArchitectureError::SagaError(format!(
                        "未知的Saga步骤: {}",
                        step.action
                    )));
                }
...
impl Default for ChaosEngineering {
    fn default() -> Self {
        Self::new()
    }
}
...
impl ChaosEngineering {
    ...
    ) -> Result<ExperimentResults, ArchitectureError> {
...
pub trait Scaler {
    ...
    ) -> Result<ScalingAction, ArchitectureError>;
        ...
        if let Some(cpu_metric) = metrics.get("cpu_usage") {
            ...
            if cpu_metric.value > self.scale_up_threshold && current_instances < self.max_instances
            {
                Ok(ScalingAction {
                    ...
                    reason: format!(
                        "CPU使用率 {}% 超过阈值 {}%",
                        cpu_metric.value, self.scale_up_threshold
                    ),
                })
            } else if cpu_metric.value < self.scale_down_threshold
                && current_instances > self.min_instances
            {
                Ok(ScalingAction {
                    ...
                    reason: format!(
                        "CPU使用率 {}% 低于阈值 {}%",
                        cpu_metric.value, self.scale_down_threshold
                    ),
                })
            } else {
                Ok(ScalingAction {
...
mod tests {
    ...
    async fn test_event_store() {
        ...
        event_store
            .save_events(aggregate_id, events.clone(), 0)
            .await
            .unwrap();
