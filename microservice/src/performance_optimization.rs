                    ...
                    match test_fn().await {
                        Ok(_) => {
                            let response_time = request_start.elapsed().as_millis() as u64;
                            {
                                ...
                                stats_guard.successful_requests.fetch_add(1, Ordering::Relaxed);
                                stats_guard.total_response_time.fetch_add(response_time, Ordering::Relaxed);
                                
                                // 更新最小和最大响应时间
                                let mut current_min = stats_guard.min_response_time.load(Ordering::Relaxed);
                                while response_time < current_min {
                                    match stats_guard.min_response_time.compare_exchange_weak(
                                        current_min, response_time, Ordering::Relaxed, Ordering::Relaxed
                                    ) {
                                        Ok(_) => break,
                                        Err(x) => current_min = x,
                                    }
                                ...
                                let mut current_max = stats_guard.max_response_time.load(Ordering::Relaxed);
                                while response_time > current_max {
                                    match stats_guard.max_response_time.compare_exchange_weak(
                                        current_max, response_time, Ordering::Relaxed, Ordering::Relaxed
                                    ) {
                                        Ok(_) => break,
                                        Err(x) => current_max = x,
                                    }
...
impl MemoryOptimizer {
    ...
    pub async fn optimize_memory(&self) -> Result<MemoryOptimizationResult> {
        ...
        let result = MemoryOptimizationResult {
            ...
            fragmentation_reduced_percent: start_stats.memory_fragmentation_percent - end_stats.memory_fragmentation_percent,
    ...
    async fn optimize_object_pooling(&self) -> Result<()> {
        ...
        for i in 0..10 {
            ...
            for _ in 0..self.config.pool_size {
                objects.push(Box::new(format!("pooled_object_{}", i)) as Box<dyn std::any::Any + Send + Sync>);
            }
...
impl ConcurrencyOptimizer {
    ...
    async fn optimize_connection_pool(&self) -> Result<()> {
        ...
        if current_connections < optimal_connections {
            self.active_connections.store(optimal_connections, Ordering::Relaxed);
        }
    ...
    async fn optimize_thread_pool(&self) -> Result<()> {
        ...
        println!("  ✅ 线程池优化完成: {} 个线程, 利用率 {:.1}%", thread_pool_size, utilization);
        Ok(())
    }
    ...
    async fn optimize_batching(&self) -> Result<()> {
        ...
        println!("  ✅ 批处理优化完成: 批次大小 {}, 效率 {:.1}%", batch_size, batch_efficiency);
        Ok(())
    }
    ...
    async fn optimize_backpressure(&self) -> Result<()> {
        ...
        if current_load > threshold * 100.0 {
            println!("  ✅ 背压优化完成: 负载 {:.1}%, 阈值 {:.1}%", current_load, threshold * 100.0);
        } else {
    ...
    async fn optimize_circuit_breaker(&self) -> Result<()> {
        ...
        if current_error_rate > threshold * 100.0 {
            println!("  ✅ 熔断器优化完成: 错误率 {:.1}%, 阈值 {:.1}%", current_error_rate, threshold * 100.0);
        } else {
...
mod tests {
    ...
    async fn test_benchmarker() {
        ...
        let result = benchmarker.run_benchmark(|| async {
            tokio::time::sleep(Duration::from_millis(10)).await;
            Ok(())
        }).await.unwrap();
